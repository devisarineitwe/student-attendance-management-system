...
if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit") {
  ...
  if (isset($_POST['update'])) {
    ...
    otherName='$otherName', admissionNumber='$admissionNumber',password='12345', classId='$classId',classArmId='$classArmId', status='$status' where Id='$Id'");
    if ($query) {
...
if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "delete") {
  ...
  // first query the database to get the details of the student being deleted
  $sql = " DELETE FROM tblstudents WHERE Id=$Id ";

  $query = mysqli_query($conn, $sql);

  if ($query == TRUE) {
    echo "<script type='text/javascript'>alert('the student has been sucessfully deleted')</script>";
                    ...
                    <div class="form-group row mb-3">
                      ...
                      <!-- SELECT CLASS -->
                        ...
                        if ($num > 0) { ?>
                          ...
                          <?php while ($rows = $result->fetch_assoc()) { ?>
                            <option value="<?php echo $rows['Id']?>" <?php if($row["classId"] == $rows["Id"]){echo 'selected';}else{echo $row["classId"];} ?> > <?php echo $rows['className'] ?> </option>
                          <?php } ?> 
                          </select>
                        <?php } ?>
                      ...
                      <div class="col-xl-6">
                        ...
                        <select required name="status" class="form-control mb-3">
                          ...
                          <option value="left" <?php if ($row["status"] == "left") {
                                                    echo 'selected = "selected"';
                                                  } ?>>Left </option>
                        </select>
                ...
                <div class="col-lg-12">
                  <div class="card mb-4">
                    ...
                    <div class="table-responsive p-3">
                      ...
                      tblstudents.lastName,tblstudents.otherName,tblstudents.admissionNumber,tblstudents.dateCreated, tblstudents.status 
                      FROM tblstudents
